...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRsdGpoamJodWh2b2hheW1xd2hrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjAzOTI0NjEsImV4cCI6MjA3NTk2ODQ2MX0.BRRNmDx8ulvKkDXRL-7bGX9uijbeRV_NXYFMmSUZ0vw';
...
export default function App() {
  const [view, setView] = useState<'home' | 'library' | 'editor' | 'settings'>('home');
  ...
  const createProjectFromDateRange = async () => {
    ...
    const filteredMedia = filterMediaByDateRange(dateRange.start, dateRange.end);
...
function HomeScreen({ projects, onOpenSettings, onOpenDatePicker, onOpenLibrary, onOpenProject }: HomeScreenProps) {
  return (
    <View style={styles.container}>
      ...
      >
        <Text style={styles.actionButtonText}>📅 Create Vlog from Date Range</Text>
      </TouchableOpacity>
          ...
          projects.map(project => (
            ...
            >
              ...
              <Text style={styles.projectDate}>
                {project.clips.length} clips • {new Date(project.updated_at).toLocaleDateString()}
              </Text>
            </TouchableOpacity>
          ))
        )}
...
function LibraryScreen({ mediaLibrary, selectedMedia, onBack, onOpenDatePicker, onToggleSelect, onCreateFromSelected }: LibraryScreenProps) {
  return (
    <View style={styles.container}>
        ...
        renderItem={({ item }) => (
          ...
          >
            <View style={styles.thumbnailContainer}>
              <View style={[
                styles.thumbnail,
                selectedMedia.find(m => m.id === item.id) && styles.selected
              ]}>
...
function EditorScreen({ project, onSaveAndBack, onRemoveClip, onStitch, onGenerateNarration, onExport, onDeleteProject }: EditorScreenProps) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.editorSection}>
        ...
        <TouchableOpacity style={styles.toolButton} onPress={() => onStitch(project.clips)}>
          <Text style={styles.toolButtonText}>🎬 Stitch Videos Together</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.toolButton} onPress={() => onGenerateNarration(project)}>
          <Text style={styles.toolButtonText}>🤖 Generate AI Narration</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.toolButton, styles.exportButton]} onPress={onExport}>
          <Text style={[styles.toolButtonText, styles.exportButtonText]}>💾 Export Vlog</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.toolButton, styles.deleteButton]} onPress={onDeleteProject}>
          <Text style={[styles.toolButtonText, styles.deleteButtonText]}>🗑️ Delete Project</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
...
function DatePickerModal({ visible, start, end, onChangeStart, onChangeEnd, onCancel, onConfirm }: DatePickerModalProps) {
  return (
        ...
        <LibraryScreen
          ...
          onCreateFromSelected={async () => {
            const newProject: Project = {
              ...
              date_range_start: new Date(selectedMedia[0].timestamp).toISOString(),
              date_range_end: new Date(selectedMedia[selectedMedia.length - 1].timestamp).toISOString(),
              clips: selectedMedia,
              created_at: new Date().toISOString(),
              updated_at: new Date().toISOString(),
              user_id: session!.user.id,
